...
let user = {
  fullname: 'Asim Ali',
  email: `asimali${Date.now()}@gmail.com`,
  username: `asimali${Date.now()}`,
  password: 'Asim123$'
};
...
test('Update User API Test', async ({ request }) => {
  ...
  const updatedUser = {
    fullname: 'Asim Ali Updated',
    email: `asimaliupdated${Date.now()}@gmail.com`,
    username: `asimaliupdated${Date.now()}`
  };
